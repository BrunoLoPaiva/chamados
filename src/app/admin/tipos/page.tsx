      ...
      <div className="bg-white  rounded-lg shadow-sm border border-neutral-200  p-8">
        <div className="mb-6">
          ...
          <p className="text-neutral-500  mt-1">
            Defina os tipos de solicitação, SLAs, o Local/Sublocalização onde se
            aplica e os Checklists do técnico.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tipos.map((tipo) => {
          ...
          return (
            <div
              key={tipo.id}
              className="bg-white  rounded-lg shadow-sm border border-neutral-200  overflow-hidden flex flex-col transition-colors"
            >
              <div className="p-5 border-b border-neutral-100 ">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-bold text-neutral-900 0 leading-tight pr-2">
                    {tipo.nome}
                  </h3>
                ...
                <div className="flex flex-wrap gap-2 text-xs mb-3">
                  <span
                    className={`px-2 py-0.5 rounded font-bold uppercase ${tipo.prioridade === "Alta" ? "bg-red-50 text-red-600  " : tipo.prioridade === "Media" ? "bg-amber-50 text-amber-600  " : "bg-emerald-50 text-emerald-600  "}`}
                  >
                    {tipo.prioridade}
                  </span>
                  <span className="px-2 py-0.5 rounded bg-neutral-100 text-neutral-600   font-bold transition-colors">
                ...
                {tipo.deptoTipos.map((dt: any) => (
                  <div key={dt.id} className="space-y-1 mt-2">
                    ...
                    {!dt.localId && (
                      <p className="text-xs text-neutral-400 00 italic">
                        Válido em qualquer local
                      </p>
                    )}
                  </div>
                ))}
              ...
              <div className="p-5 flex-1 bg-neutral-50/50 /30 transition-colors">
                <h4 className="text-xs font-bold text-neutral-500  uppercase tracking-wider mb-3">
                  Checklist (Ações do Técnico)
                </h4>
                ...
                <ul className="space-y-2 mb-4">
                  {acoesTipo.map((acao) => (
                    <li
                      key={acao.id}
                      className="flex justify-between items-center text-sm bg-white  border border-neutral-200  p-2 rounded-lg transition-colors group"
                    >
                  ...
                  {acoesTipo.length === 0 && (
                    <li className="text-sm text-neutral-400 00 italic">
                      Sem checklist para este tipo.
                    </li>
                  )}
                </ul>
                ...
                <form action={createAcaoParaTipo} className="flex gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 text-sm bg-white  border border-neutral-300  rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none transition-colors"
                  />
                  <button
                    type="submit"
                    className="px-3 py-1.5 bg-neutral-200  hover:bg-neutral-300  text-neutral-700  text-sm font-medium rounded-lg transition-colors flex items-center justify-center"
                  >
